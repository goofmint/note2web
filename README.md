...
添付画像・手書き描画は R2 / S3(いずれも S3 互換 API)へアップロードし、本文中の参照を `public_base_url` + キーの URL に差し替えます(**note.com 向けの画像だけは例外**で、R2 / S3 は使わず `note.workspace` の `images/` フォルダへローカルコピーします。詳細は下記「note.com」を参照)。
...
- **画像は `noet` 自身のアップロード機能で自動的に配信されます**。note.com の編集画面(ProseMirror)は Markdown の画像記法 `![]()` を解釈しないため(外部 URL のまま送ってもリテラルなテキストとして表示されてしまいます)、note2web は画像添付を R2 / S3 へはアップロードせず、代わりに `note.workspace` 直下の `images/` フォルダへコピーし、本文の参照をそのローカル相対パス(`![alt](./images/<識別子>.png)` 等)に書き換えます。`noet create`/`update` の実行時、この相対パス参照を `noet` 自身が検出して note.com へアップロードし、アップロード後の URL に本文中の参照を差し替えたうえで公開します
  - **対応する画像形式は jpg / jpeg / png / gif / webp のみ**です(`noet` 側の制約)。これ以外の拡張子の画像添付を含むノートは、その添付だけが原因でそのノートが `failed` になります(エラーメッセージに拡張子と対応形式の一覧が出ます)。画像を別形式に変換するか、そのノートだけ note.com 以外のサービスへ配信してください
  - この `noet` の画像アップロード機能は **upstream で「統合テスト未実施」の未検証機能**であり、note2web 側でも実機での動作確認はできていません。失敗した場合は他の失敗と同様にそのノートのみが `failed` として隔離され、状態は更新されません(次回実行で自動的に再試行されます)
  - 画像を多数含む記事では、拡張機能側の1コマンドあたりのタイムアウト(60秒)に接近する可能性があります(上記「レート制御」相当の注意点。詳細は design.md §5.7 参照)
  - `note.workspace/images/` に、もはやどのノートからも参照されなくなった過去の画像ファイルが残ることがありますが、無害です(`noet` は毎回そのノートが実際に参照する画像だけをアップロードします)。手動で削除しても問題ありません
