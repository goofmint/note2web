...
- **note.com**(非公式 API を直接呼び出し。※後述の注意事項あり)
...
- **Qiita / dev.to / note.com / はてなブログを使う場合**: 追加の外部 CLI は不要です(API を直接呼び出します)。トークン・セッション cookie は環境変数で渡します(後述)。note.com は非公式 API のため、事前にブラウザから手動でセッション cookie を取得しておく必要があります(詳細は [note.com](#notecom) の節を参照)
...
- 各サービスのトークン・API キー・note.com のセッション cookie は、いずれも設定 YAML の `*_env` が指す環境変数名でこのファイルに書きます(値そのものは設定 YAML には一切書きません。FR-30)
...
添付画像・手書き描画は R2 / S3(いずれも S3 互換 API)へアップロードし、本文中の参照を `public_base_url` + キーの URL に差し替えます(**note.com 向けの画像だけは例外**で、R2 / S3 は使わず note.com 自身の presigned アップロード API へ配信時点で直接アップロードします。詳細は下記「note.com」を参照)。
...
```yaml
note:
  session_cookie_env: NOTE_SESSION_COOKIE   # セッション cookie を読む環境変数名(サンプルは同名だが変更可)
```

note.com は公式 API を提供していないため、note2web は非公式 API(「note非公式APIを徹底調査|2026年版エンドポイント一覧完全版」https://note.com/marie_222/n/n6a10366298b0 の調査結果に基づく wire contract)を直接呼び出します。認証は note.com のセッション cookie(`_note_session_v5`)で行い、プログラムによるログイン(cookie の自動取得)は note.com 側の reCAPTCHA v3 でブロックされているため、cookie の値は**利用者がブラウザから手動で取得**する必要があります:

1. 通常のブラウザ(Chrome / Safari 等)で note.com にログインする
2. DevTools を開く(Chrome: `F12` または `Cmd+Option+I`)→「Application」タブ →「Cookies」→ `https://note.com` を選択
3. `_note_session_v5` という名前の cookie を探し、その値をコピーする
4. [env ファイル](#env-ファイルの自動読み込みdoctor--sync-共通)に、`note.session_cookie_env` が指す環境変数名(既定 `NOTE_SESSION_COOKIE`)としてこの値を設定する

この cookie は数か月単位で有効ですが、**失効した場合は同じ手順で再取得し、env ファイルの値を更新してください**(失効すると `sync` が認証エラー(`NoteAuthError`)でそのノートを `failed` として扱います。エラーメッセージにも同じ再取得手順が表示されます)。

**注意**: 非公式 API のため、note.com 側の仕様変更やアクセス遮断が予告なく発生する可能性があります。低頻度の個人利用の範囲にとどめ、大量・高頻度の自動投稿には使わないでください。cookie が有効な限り他サービスと同様に完全無人実行できますが(NFR-03)、これは note.com が公式にサポートする利用方法ではない点をご理解のうえご利用ください。
...
note.com は公式 API を提供していないため、note2web は非公式 API(「note非公式APIを徹底調査|2026年版エンドポイント一覧完全版」https://note.com/marie_222/n/n6a10366298b0 の調査結果に基づく wire contract)を直接呼び出して配信します。**完全無人実行が可能です**(セッション cookie が有効な限り、他サービスと同じく cron / launchd から対話なしで動作します)。

- **認証**: セッション cookie(`_note_session_v5`)による認証です。取得手順は上記「[設定ファイル](#設定ファイル)」の「note.com」節を参照してください。cookie が失効している(または未設定の)場合、そのノートの配信のみが認証エラー(`NoteAuthError`)で `failed` になります(他ノートの処理は継続します)。エラーメッセージに再取得手順が表示されるので、それに従ってブラウザから cookie を再取得し、env ファイルの値を更新したうえで再実行してください
- **新規作成・更新**: `remoteId`(状態 JSON)が無いノートは新規作成、あれば更新します。dev.to/はてなのような既存記事とのタイトル一致照合は行いません(Qiita と同じ単純な契約)
- **画像**: 本文中の画像添付は note.com 自身の presigned アップロード API へ配信のたびに直接アップロードします(R2 / S3 は経由しません)。**対応する画像形式は jpg / jpeg / png / gif / webp のみ**です。これ以外の拡張子の画像添付を含むノートは、その添付だけが原因でそのノートが `failed` になります(エラーメッセージに拡張子と対応形式の一覧が出ます)。画像を別形式に変換するか、そのノートだけ note.com 以外のサービスへ配信してください
  - **注意**: 画像だけを差し替えても、本文(タイトル・タグ・テキスト)が変わらない限り自動的には再配信されません(冪等判定のハッシュがタイトル・タグ・本文 Markdown を対象とし、画像バイト列そのものは含まないため)。この場合、FR-15 の skip 判定により再配信されず note.com 側には旧画像が残ったままになります。画像を更新したい場合は本文側にも何らかの変更を加えるか、状態 JSON の該当ノートの `contentHash` を手動で変更して強制的に再配信してください
- **配信失敗時に空の下書きが残る場合がある**: 下書きの予約(`POST /api/v1/text_notes`)に成功した後、画像アップロードや公開(`PUT`)が失敗すると、その下書きは note.com 側にそのまま残ります。note2web はこの残存下書きを次回実行時に再利用せず、**新しい下書きを新規に作成**します。残存下書きは公開されていないため外部には表示されませんが、note.com のダッシュボード上には残るので、不要であれば手動で削除してください
- **非公式 API に関する注意**: note.com は公式にこの利用方法をサポートしていません。仕様変更やアクセス遮断が予告なく発生する可能性があるため、**低頻度の個人利用の範囲にとどめてください**。大量・高頻度の自動投稿や商用の大規模運用には使わないことを強く推奨します
- **過去バージョンからの移行(noet ワークスペース時代の状態ファイル)**: issue #86 以前の note2web は状態ファイルの `target` に旧 noet ワークスペースのパス文字列を記録していました。現行バージョンでは `target` が固定値 `"note.com"` になったため、この不一致により既存の状態ファイルをそのまま使うと `target` 検証エラー(exit 2)になります。**既存の `remoteId`(記事 ID)を引き継いで重複記事の作成を避けるには、状態ファイルの `target` を手動で `"note.com"` に書き換えてから再実行してください**(Qiita の移行手順と同様です)
...
note.com 向けの構成も他サービスと同様に launchd から完全無人実行できます。セッション cookie が env ファイルに設定されている限り、追加の前提(ログイン済みブラウザ等)は不要です——ただし cookie が失効した場合は、ブラウザから手動で再取得して env ファイルを更新するまで note.com への配信のみ失敗し続けます([note.com](#notecom) 節を参照)。
...
- **note.com への実際の投稿**: 非公式 API の wire contract は HTTP モックで検証済みですが、実際のセッション cookie を使った note.com への記事作成・更新・画像アップロードの成功は未確認です(design.md §5.7 NotePublisher 節「実機確認課題」参照)。この API 自体が非公式であるため、note.com 側の仕様変更・遮断によって動作しなくなる可能性が常にある点にもご注意ください
