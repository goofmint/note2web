...
図はフェーズの俯瞰と主要なブロッキング関係のみを示す。タスク単位の正確な依存は §2 の依存列を唯一の正とする。
...
| T-13 | AssetUploader(R2 / S3) | M | T-07, T-11 | FR-13, FR-14, FR-17, §5.5 |
...
| T-28 | 実機 E2E 検証(Zenn / M1 ゲート) | M | T-15, T-17 | §12, M1 |
...
- **受け入れ条件**: 各イベントの出力形式のユニットテスト。「何を・いつ・どこへ・成否」のフィールドが揃っている。全イベントが stdout へ出力されること、`log.file` 設定時のみファイルへも追記されること、追記時に既存内容が保持されることをテストする(設定なしの場合はファイルが作られないことも確認)

#### T-04 設定 YAML ローダとスキーマ検証(M)
- §7 のスキーマを zod で定義。サービス別の必須項目、`*_env` キーの環境変数存在チェック、秘匿情報の直書き拒否、`timezone` 既定値
- **受け入れ条件**: §7 のサンプル設定が全サービス分パースできる。以下の各ケースで正常系・不正系をテストし、不正時はどのキーが問題か明示して exit 2 になる: (1) サービス別必須項目の欠落、(2) `*_env` が指す環境変数の未設定、(3) 秘匿情報の直書き(拒否される)、(4) `timezone` 省略時に `Asia/Tokyo` が適用される
...
- **受け入れ条件**: **子プロセスを生成する**ハング状態のダミープロセスに対し、プロセスグループごと終了すること(子の取り残しなし)、SIGTERM 猶予後に SIGKILL へ移行すること、stdout / stderr が取得できること、失敗分類(`timeout` / `exit_code` / `signal`)がログに乗ることをテストする
...
- **受け入れ条件**: 検証不一致で exit 2。保存途中クラッシュを模擬しても既存ファイルが壊れない。read-your-writes 契約のテスト: 同一実行内の書き込みが直後の読み取りに即時反映される、ディスク読み込みは起動時の1回だけ(以後ファイルを外部変更しても再読み込みされない)、保存が「アセット成功時」「ノート配信確定時」の2ポイントで行われる
...
- **受け入れ条件**: golden 比較で次をすべて検証する: 表・チェックリストの GFM 変換、添付・描画参照のプレースホルダ化、タイトル行の除去、未対応要素のテキスト化。未対応要素については警告ログの内容(どのノートのどの要素か)もテストする
...
- **受け入れ条件**: モック S3 で「初回アップロード」「同一実行内の重複参照」「実行をまたいだ再実行」のいずれも二重アップロードが起きない。加えて、本文中の全プレースホルダが最終 URL に置換され残存しないこと、アップロード成功ごとに StateStore の `assets` へ保存されることを個別に検証する
...
#### T-28 実機 E2E 検証(Zenn / M1 ゲート)(M)
- モックではなく実環境での検証: 実機 macOS の Apple Notes(表・チェックリスト・画像・絵文字タイトルを含むノート)を用意し、実際の Zenn 連携リポジトリと R2 / S3 バケットに対して `note2web sync` を実行する
- **受け入れ条件**: (1) 実機 Apple Notes から Zenn リポジトリへ PR が作成され、記事が Zenn の形式(slug・frontmatter)を満たす、(2) アセットが R2 / S3 にアップロードされ本文 URL が有効、(3) 同一入力での2回目実行が全件 skip になり空 PR を作らない、(4) 手順と結果を記録し、fixture との差異があれば design.md / 各タスクへフィードバックする

## 4. マイルストーン

| マイルストーン | 含むタスク | 到達点 |
|---|---|---|
| **M1: Zenn E2E** | T-01〜T-17, T-28 | T-28 の受け入れ条件を満たす: 実機の Apple Notes から Zenn リポジトリへ PR が作られ、2回目実行で skip される(最小の縦切り) |
