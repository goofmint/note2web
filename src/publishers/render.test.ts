...
describe('renderGenericArticle', () => {
  it('produces a deterministic content hash for identical input', () => {
    ...
    const a = renderGenericArticle({ note, markdown: 'body text', config: API_CONFIG });
    const b = renderGenericArticle({ note, markdown: 'body text', config: API_CONFIG });
  ...
  it('changes the content hash when the body changes', () => {
    const note = buildNote();
    const a = renderGenericArticle({ note, markdown: 'body text', config: API_CONFIG });
    const b = renderGenericArticle({ note, markdown: 'different body text', config: API_CONFIG });
    expect(a.contentHash).not.toBe(b.contentHash);
  });

  it('includes title and tags in the frontmatter block', () => {
    const note = buildNote({ title: 'My Title', tags: ['#a', '#b'] });
    const { artifact } = renderGenericArticle({ note, markdown: 'body', config: API_CONFIG });
  ...
  it('carries noteUuid and title through to the RenderedArticle', () => {
    ...
    const article = renderGenericArticle({ note, markdown: 'body', config: API_CONFIG });
  ...
  it('derives artifactPath as "<git.output_dir>/<uuid>.md" for git-mode services (design.md §8 example)', () => {
    ...
    const article = renderGenericArticle({ note, markdown: 'body', config: GIT_CONFIG });
