 ...
 *
 * **更新時に slug を上書きする副作用(既知の制約、意図的に対処しない)**: 手順5の `PUT` は
 * 新規作成・更新のいずれでも同じフルペイロードを送るため(差分 API が無い、元記事)、更新の
 * たびに `slug: slug-<note_key>`(`buildNotePublishPayload` 参照)が送られる。利用者が
 * note.com の Web エディタ上でその記事の slug を独自の値へ変更していた場合、次回の note2web
 * 配信でその値は `slug-<note_key>` へ上書きされる。note.com には「自分の記事一覧」相当の
 * API が無く(上記「応答不明時の重複防止」参照)現在の slug を安価に読み取る手段が無いため、
 * この上書きを回避する実装上の対処はしていない。ただし出典記事(モジュール冒頭)の調査に
 * よれば、note.com の記事 URL は常に `https://note.com/<urlname>/n/<key>`(note key ベース)
 * で決まり、slug フィールドは URL・ルーティングのいずれにも影響しない vestigial な値である
 * ため、実際の実害は無いと考えられる。
 *
 ...
 *
 * **孤児下書きの残存(既知の制約)**: `createDraft()`(手順1)の成功後、画像アップロード
 * (手順3)または公開 `PUT`(手順5)のいずれかが失敗すると、その時点で予約済みの下書き
 * (および既にアップロード済みの画像)は note.com 側にそのまま残る。状態 JSON への `remoteId`
 * の保存は手順5の `PUT` 成功後にのみ行われる(モジュール冒頭「作成・更新フロー」参照)ため、
 * 次回実行はこの孤児下書きを再利用せず、`prev.remoteId` が無いままの新規ノートとして扱い、
 * 新しい下書きを新規に予約する。孤児下書きは公開されていないため note.com の公開ページとして
 * は表示されないが、note.com のダッシュボード(下書き一覧)には残るため、不要であれば利用者
 * が手動で削除する必要がある(README「note.com」節に同じ注記あり)。
 *
