 ...
 *
 * `note2web doctor` / `note2web sync` を対話シェルから直接実行した場合、かつて CLI 自身は
 ...
 *
 * **launchd 経由での実行における役割(issue #71)**: `note2web init` が生成する plist は
 * `node` を直接起動し、`EnvironmentVariables` には非秘匿の `PATH` しか含めない(以前あった、
 * env ファイルをシェルとして評価してから起動するラッパースクリプトは廃止済み)。したがって
 * launchd 経由の実行では、トークン等の秘匿情報を `process.env` へ載せる手段はこのモジュールの
 * 読み込みだけになる——本モジュールは単なる「対話シェルとの整合性のための補助」ではなく、
 * launchd 実行時の秘匿情報の唯一の入力経路である。
 *
 * 本モジュールはシェルとして評価しない。`$VAR` や `` `cmd` `` のようなシェル構文
 * (コマンド置換・変数展開・エスケープシーケンス解釈)は一切行わず、`=` の右辺を
 * リテラルな文字列としてそのまま扱う(前後の空白の除去、および一致するクォート1組の
 * 除去のみ行う)——単純な `NAME=value` 形式のみを対象とする、という割り切った実装で
 * あることに注意。
 *
