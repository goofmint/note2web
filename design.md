...
- **外部ツールはサブプロセス**: `apple_cloud_notes_parser`・`gh` は外部 CLI として呼び出す。ライブラリとしてリンクしない(issue #82: Qiita は当初 `@qiita/qiita-cli` サブプロセス経由だったが、qiita-cli の `publish` が投稿前に利用者の全記事を無条件同期する実装のため大規模アカウントでタイムアウトし、Qiita API v2 を dev.to と同じ HTTP 直叩き方式へ移行した。§5.7 QiitaPublisher 参照)。note.com も当初は外部 CLI `noet` のサブプロセス経由を想定していたが、issue #86 で同じ理由(無人サブプロセス実行との非両立)により非公式 API 直叩き方式へ移行した(§5.7 NotePublisher 参照)
...
| note.com | **確認済み(§13-4/§13-6 解消)。ただし重大な設計前提の崩れあり**。`noet`(kako-jun/noet, commit `e3a8562`)は要件調査時点(README.md の記載)とは**別物の内部アーキテクチャに移行済み**: 現行ソース(`apps/cli/src/cli.rs` 等)には note.com の非公式 API を直接叩くコードは無く、`Note.comのAPIは一切使用しない。すべての操作はブラウザ拡張機能を経由してDOM操作で行う`(`CLAUDE.md:19`)という設計に全面移行している。CLI はローカルの Chrome 拡張機能と `ws://127.0.0.1:9876` の WebSocket で通信し、拡張機能側が**実際にログイン中の人間のブラウザ**で note.com のページを開いて DOM 操作(フォーム入力・ボタンクリック)を行う(`apps/extension/src/background.js`)。README.md が案内する環境変数認証(`NOET_SESSION_COOKIE` 等)やレート制御(500ms 固定)は**現行コードには存在しない旧アーキテクチャの記述**(ソース grep で該当箇所ゼロ。詳細は §13-4)。この結果、**`noet` はサーバー / cron 上でヘッドレスに動かせる「gh・qiita-cli 相当のサブプロセス CLI」ではない**。加えて画像は note.com 側の ProseMirror エディタが Markdown 画像記法 `![]()` を解釈しないため、外部 URL をそのまま本文に埋め込んでも画像としては表示されない(noet 自身の調査結果 `docs/IMAGE_UPLOAD_INVESTIGATION.md` より。§13-6)。**この2点は note2web の当初設計(サブプロセス呼び出し・R2/S3 URL そのまま埋め込み)の前提を崩す。T-25(issue #30)で対応方針を決定・実装済み: (1) 認証前提が満たされない実行(cron 等)ではノート単位の failed として扱う自動実行、(2) 画像は noet 自身の画像アップロード機能をローカルファイル参照で使う(option (a)。利用者決定 2026-08-21。旧 option (b)「画像を含むノートを明示的に failed とする」から変更)。詳細は §5.7 NotePublisher 節を参照** |

**issue #86 追記**: 上記(§13-4/§13-6)の noet 依存はすべて撤去済み。note.com は「note非公式APIを徹底調査|2026年版エンドポイント一覧完全版」(https://note.com/marie_222/n/n6a10366298b0)の調査結果に基づき、非公式 API を本体(`src/publishers/note-client.ts`)から直接叩く方式へ移行した。認証はセッション cookie(`_note_session_v5`。利用者がブラウザから手動取得し `note.session_cookie_env` が指す環境変数へ設定)で行うため、旧 noet 時代の「サーバー上で完結する認証手段が存在しない」という構造的制約は解消し、cookie が有効な限り note.com も完全な無人実行が可能になった。詳細は §5.7 NotePublisher 節(全面書き換え済み)を参照。§13-4/§13-6 の記述は noet 依存時代の調査記録として歴史的に残す。
...
| 地の文の `` `code` ``(バッククォート対。下記「インラインコード認識」) | 逐語のインラインコード | (実機修正) |
| リンクテキストが URL そのものの `<a href="URL">URL</a>`(URL 貼り付け) | 素の URL テキスト(下記「オートリンクの素の URL への展開」) | (実機修正) |
...
- **インラインコード認識(実機公開で発覚した不具合の修正。コードフェンス認識のインライン版)**: ノート本文に地の文として書かれたインラインコード(`` `code` `` のバッククォート対)を、逐語の `inlineCode` ノードとして認識する(認識しないと `remark-stringify` が `` \`code\` `` とエスケープしてしまう)。hast→mdast 変換後・直列化前の後処理(`recognizeInlineCode`)として、mdast ツリー全体の `text` ノードを走査し、「バッククォート対に挟まれた、バッククォート・改行を含まない1文字以上の内容」を `inlineCode` へ分割・差し替える
  - 対になっていないバッククォート(奇数個の余り)は従来どおりエスケープされたまま残す(対の位置を推測しない。コードフェンス認識の「閉じ位置を推測しない」と同じ方針)
  - 改行(`<br>` による行分割・text ノード内の `\n`)を跨ぐ対は認識しない
  - `inlineCode` の内容は完全に逐語(内容にバッククォートは含まれ得ないため、`remark-stringify` の区切り伸長も不要)
- **オートリンクの素の URL への展開(実機 Zenn 公開に向けた修正)**: リンクテキストが URL そのものと完全一致するリンク(Apple Notes で URL を貼り付けたときの `<a href="URL">URL</a>`)は、`remark-stringify` の既定ではオートリンク `<URL>` として直列化されるが、Zenn のリンクカードは**行全体が素の URL** のときにのみ発動するため、素の URL テキストとして出力する。hast→mdast 変換後・直列化前の後処理(`unwrapAutolinks`)として、該当する `link` ノードを `html` ノード(逐語出力)へ差し替える——`text` ノードだと URL 中の `_` 等が `\_` にエスケープされ得るため。リンクテキストが URL と異なる通常のリンク・`title` 付きリンク・アセットプレースホルダへの参照は対象外(従来どおり `[テキスト](URL)`)
...
**issue #86 で noet サブプロセス方式を全面撤去し、note.com 非公式 API を直接叩く方式へ移行した。以下は移行後の契約。旧 noet 方式の調査記録(§13-4・§13-6)は歴史的経緯として §13 に残す(下記「移行の背景」参照)。**

- **移行の背景(issue #86)**: 旧実装(issue #30/#85)は外部 CLI `noet` のサブプロセス実行に配信を委ねていたが、`noet` は要件調査時点とは異なる内部アーキテクチャ(ローカルの Chrome 拡張機能と WebSocket で通信し、拡張機能側が実ブラウザ上で DOM 操作を行う構成、§13-4)へ移行済みであることが判明し、サーバー上で完結する認証手段が存在しないため無人(cron/launchd)実行が構造的に不可能だった。加えて画像は `noet` 自身のローカルファイル参照経由のアップロード機能に依存しており、ワークスペースディレクトリという概念自体が note2web の他サービスと非対称だった。issue #86 はこれらの制約を、`noet` への依存を完全に取り除き、note.com の非公式 API を本体(`src/publishers/note-client.ts`)から直接叩く方式へ置き換えることで解消する。**出典**は「note非公式APIを徹底調査|2026年版エンドポイント一覧完全版」(https://note.com/marie_222/n/n6a10366298b0、以下「元記事」)——note.com アカウント・実ブラウザを使ったブラウザ DevTools ネットワークタブの観察に基づく非公式の調査記事であり、note.com 自身が公開する仕様書ではない(§12「実機確認」参照)
- **認証**: 全リクエストに `Cookie: _note_session_v5=<値>` を付与する(元記事)。note.com へのプログラムによるログインは reCAPTCHA v3 でブロックされているため、cookie の値は利用者がブラウザの DevTools(Application → Cookies)から手動で取得し、設定 `note.session_cookie_env` が指す環境変数(FR-30。値そのものは設定 YAML に書かない)へ設定する。cookie は数か月単位で有効(元記事)。`GET /api/v2/current_user` がログイン状態の確認を兼ね、`urlname`(記事 URL の組み立てに使う)を返す
- **作成・更新フロー(元記事)**:
  - `POST /api/v1/text_notes`(空ボディ)で下書きを予約すると、応答に数値 `id` と note key(`n...`)が含まれる
  - `PUT /api/v1/text_notes/{numeric_id}` にフルペイロードを送ると記事が公開される(`status: "published"`)。**同じ PUT が既存記事の更新にも使われる**——差分 API は無く、常にフルペイロードを送る
  - `remoteId`(状態 JSON、数値 ID を文字列化)が無いノートは `POST` で下書き予約 → `PUT` で公開。`remoteId` があれば照合不要でそのまま `PUT` で更新する(dev.to/はてなのようなタイトル一致照合は行わない——Qiita と同じ単純な契約。下記「応答不明時の重複防止」参照)。更新時に必要な note key(スラグの組み立てに使う)は状態 JSON に保存していないため、`prev.url`(`https://note.com/<urlname>/n/<key>` 形式)の末尾セグメントから再導出する
- **本文**: `free_body` は ProseMirror 由来の HTML で、各ブロック要素が `<p name="UUID" id="UUID">…</p>` のように `name`/`id` に同一の UUID を持つ形(元記事)。変換済み Markdown(mdast)からこの形の HTML を組み立てる(`src/publishers/note-html.ts`)。要素マッピング: 段落→`<p>`、見出し→`<h2>`/`<h3>`(note.com は見出しレベルを2段しか持たないため丸める)、リスト→`<ul>`/`<ol>`+`<li>`、コードブロック→`<pre><code>`、インラインコード→`<code>`、リンク→`<a href>`、引用→`<blockquote>`、水平線→`<hr>`、強調/強い強調→`<em>`/`<strong>`、改行→`<br>`。表・脚注・生 HTML ノード・参照形式リンク/画像等、対応要素の無いものはテキスト化して劣化させる(詳細は `note-html.ts` 冒頭 JSDoc)
- **画像(本文)のアップロード(元記事)**: 2段階——(1) `POST /api/v3/images/upload/presigned_post` で S3 の presigned POST フィールド(`data.action` = S3 URL、`data.post` = フィールド一式。`x-amz-security-token` を含む)を取得、(2) `data.post` の全フィールドを**受け取った順序のまま**マルチパートフォームへコピーし(`x-amz-security-token` の欠落は 403 InvalidAccessKeyId になる)、`file` パートを**最後**に追加して `data.action` へ POST する(成功は 204)。埋め込みは `<figure name="UUID" id="UUID"><img src="https://assets.st-note.com/img/<KEY>" alt="…"></figure>`(`<KEY>` の導出方法は実機確認課題、下記参照)。本文中の画像プレースホルダ(`note2web-asset://<identifier>`)はアセット解決段階(`assets/uploader.ts`)では未解決のまま残し(note.com は R2/S3 の公開 URL を受け付けないため、対応する画像バイト列を直接アップロードする必要がある)、配信時点(`src/publishers/note.ts` の `publish()`)で対応する添付ファイルを読み、note.com へアップロードしてから本文 HTML へ解決する
- **500 罠(元記事が報告する、次の4項目を満たさないと空ボディで 500 エラーになる)**: (1) `image_keys` は本文中に現れる全画像の `<KEY>` を出現順に列挙し、画像があるのに空配列を送ってはならない。(2) `lead_form`/`line_add_friend` は `null` にできず、`{"is_active": false, "consent_url": ""}` / `{"is_active": false, "keyword": "", "add_friend_url": ""}` を送る。(3) `body_length` は HTML 長ではなく可視テキストの Unicode コードポイント数。(4) `slug` は空にできず、既定 `slug-<note_key>` を使う。実装(`src/publishers/note.ts`/`note-client.ts`)はこの4項目をすべて満たしたペイロードを常に組み立てる
- **contentHash と画像アップロードの独立性(design 判断)**: `renderNoteArticle` は本文 Markdown を画像プレースホルダ未解決のまま title/tags と連結してハッシュ化する(dev.to/Qiita と同じ方針)。そのため、同じ添付 identifier のまま画像バイト列だけを差し替えても `contentHash` は変わらず、本文・タイトル・タグが不変な限り次回実行は skip になる——画像だけの差し替えは自動では再配信されない(CodeRabbit 実装プランが明示的に選んだトレードオフ。詳細は `src/publishers/note.ts` 冒頭 JSDoc)
- **応答不明時の重複防止**: 元記事は note.com の「自分の記事一覧」API を明らかにしていないため、dev.to/はてなのようなタイトル一致照合は行わない——Qiita と同じ単純な契約(`prev.remoteId` の有無だけで新規作成/更新を振り分ける)。POST(下書き予約・画像アップロードの presigned/S3)は自動リトライしない。PUT(公開/更新)は同一内容の再送が冪等なので、接続系エラーに限り1回だけ再試行してよい
- **実機確認課題(§12 参照。いずれも実装を1箇所に閉じ込めてあり、実機確認で判明した内容に応じてその関数だけを差し替えればよい)**:
  - (a) hashtags の正確な wire 形式 — 初期実装は `[{ name: "#タグ" }]`(`src/publishers/note.ts` の `buildNoteHashtags`)
  - (b) 書き込み系リクエストに追加ヘッダが要るか — `Origin: https://note.com` / `Referer: https://note.com/` / `X-Requested-With: XMLHttpRequest` を固定で付与している(`src/publishers/note-client.ts` の `buildNoteHeaders` に集約)
...
- `remoteId` の無いノートを新規作成する**前に、既存記事の照合**を行う(**Qiita・note.com は例外**、下記):
  ...
  - note.com: **照合を行わない**(issue #86。元記事は note.com の「自分の記事一覧」API を明らかにしていないため、Qiita と同じ単純化を採用)。`prev.remoteId` があれば `PUT`(更新)、無ければ draft 予約 `POST` → `PUT`(新規作成)とするだけ
- 照合結果の扱い(dev.to・はてなのみ): **ちょうど1件一致**した場合のみその ID を `remoteId` に採用し、更新として配信する。**0件**なら記事は未作成と判断して新規作成する。**複数一致**の場合は誤った記事への紐付けや重複作成を避けるため、そのノートを failed とし状態を更新しない(警告ログを出し、手動での解決を促す)
...
| note | `note.session_cookie_env` が指す環境変数のみ(サンプルでは `NOTE_SESSION_COOKIE`。API 直接、CLI 不要。issue #86 で noet サブプロセス方式(`NOET_PATH` の実在確認・同一マシン上のログイン済みブラウザ + noet 拡張機能の前提)を廃止) |
...
- **サブプロセス実行の共通規約**(`apple_cloud_notes_parser` / `gh` / `git`。issue #82 で Qiita、issue #86 で note がこの対象から外れ、dev.to と同じ HTTP タイムアウト30秒の規約に従う。§5.7 QiitaPublisher/NotePublisher 参照): 各コマンドにタイムアウトを設ける(parser: 15分、その他: 5分)。超過時はプロセスグループごと SIGTERM を送り、10秒待って残存すれば SIGKILL。失敗は `timeout` / `exit_code` / `signal` に分類し、failed ログの `error` に記録する。正常・異常いずれの終了経路でも一時ディレクトリの削除とロック解放を必ず実施する(プロセス自体のクラッシュで実施できなかった場合は、次回実行の stale ロック回収で回復する)
...
note:
  session_cookie_env: NOTE_SESSION_COOKIE   # issue #86: 非公式 API 直叩き方式へ移行し workspace は廃止
hatena:
  hatena_id: example
...
```json
{
  "version": 1,
  "service": "zenn",
  "target": "配信先の識別子。Git モード: repo_path、hatena: blog_id、qiita / devto / note: API ホスト（issue #82: qiita は workspace から API ホスト固定値へ、issue #86: note も workspace から API ホスト固定値へ変更）",
  "notes": {
    "5c1c2c3d-…-uuid": {
...
- **実機確認**(CI 不能なもの): note.com 非公式 API の実 cookie での認証・記事作成・画像アップロード(issue #86)、はてな AtomPub での `text/x-markdown` 入稿(T-23 時点の状況は §13-5 参照。wire contract 自体の実装と HTTP モックでの検証は完了しており、残るのは実際のはてなブログへの入稿確認のみ)、**Qiita API v2 の実トークンでの認証・記事公開**(issue #82 以降。HTTP モックでの wire contract 検証は完了)。§13 の項目と対応
  - note.com(issue #86)は wire contract 自体を HTTP モック(`test/publishers/note.test.ts`)で検証済みだが、その wire contract 自体が「note非公式APIを徹底調査|2026年版エンドポイント一覧完全版」(https://note.com/marie_222/n/n6a10366298b0)というブラウザ DevTools 観察に基づく非公式の調査記事に由来し、note.com 自身が公開する仕様書ではない。本タスクの実行環境には note.com アカウントも実ブラウザも無いため、実際の cookie を使った認証・記事作成・更新・画像アップロードの成功、および実機確認課題(a)〜(d)(§5.7 NotePublisher 節)は未確認のまま残る。旧 noet 依存時代の実機確認記録(§13-4・§13-6)は歴史的経緯として残すが、現行実装(非公式 API 直叩き)には適用されない
...
**issue #86 で撤去済み**: 以下は noet 依存時代(§13-4)の調査記録として歴史的に残す。現行実装(note.com 非公式 API 直叩き)には適用されない——現行の契約は §5.7 NotePublisher 節を参照。
...
**issue #86 で撤去済み**: 以下は noet 依存時代(§13-6)の調査記録として歴史的に残す。現行実装(note.com 非公式 API 直叩き)は R2/S3 の公開 URL をそのまま埋め込む設計そのものを採用しておらず(画像は presigned アップロード API へ直接送る)、下記の懸念は現行実装には適用されない——現行の画像アップロードの契約は §5.7 NotePublisher 節を参照。
...
**4・6(note.com / noet)はさらに深刻で、「未決事項の解消」に留まらず §1 の実行モデル(cron / launchd からの無人実行)そのものが note.com には適用できないという結論に至った**。noet(commit `e3a8562`)は README.md / CHANGELOG.md が説明する「環境変数認証 + 非公式 API 直叩き」のアーキテクチャから、現行ソースでは「ログイン済みの実ブラウザ + Chrome 拡張機能を DOM 操作で遠隔操作する」アーキテクチャへ全面移行済みであり、この移行後のアーキテクチャにはサーバー上で完結する認証手段が存在しない(§13-4)。加えて note.com の編集画面は Markdown の画像記法を解釈しないため、外部 URL をそのまま埋め込む当初設計はリテラルテキストの露出という形で失敗する可能性が高い(§13-6)。**T-25(issue #30)でこの決定を行い実装した**: 半自動モードへ後退させる案ではなく、自動実行を維持したうえで「認証前提が満たされない実行は当該ノートを failed にする」(NFR-06 の局所化に委ねる)を選び、画像は当初 §13-6 の案(b)(画像を含むノートを明示的に failed とする)を採用した。**その後 2026-08-21、利用者の実機要求により案(a)(noet 自身の画像アップロード機能をローカルファイル参照で使う)へ変更した**(現行の決定・詳細・根拠は §5.7 NotePublisher 節「画像」項を参照。本節(§13-6)は変更前の調査時点の記録として残す)。

**issue #86 追記**: 上記の T-25 時点の決定(自動実行 + ノート単位 failed 局所化、noet 自身の画像アップロード機能の利用)はいずれも noet 依存を前提としていたが、issue #86 で noet 依存そのものを撤去し、note.com の非公式 API を直接叩く方式へ移行した。これにより「サーバー上で完結する認証手段が存在しない」という上記の根拠(§13-4)は解消し、cookie が有効な限り note.com も他サービスと同じ完全な無人実行が可能になった(NFR-03/FR-31 の note.com 例外は撤廃)。本節(§13-4・§13-6)は noet 依存時代の調査記録として歴史的に残すのみで、現行実装には適用されない。現行の契約・実機確認課題は §5.7 NotePublisher 節を参照。
