...
**調査済み(§13-4/§13-6)。ただし他の Publisher とは前提が大きく異なるため、実装(T-25)前に方針決定が必要(下記「推奨対応」参照)。**

- **コマンド体系(§13-4)**: `noet` に「公開」を意味する単一コマンドは無い。新規は `noet create <file> [--draft]`、更新は `noet update <key> <file> [--draft]` を使う(`--draft` を付けない = 公開)。記事ファイルは `noet` ワークスペース内の決まったディレクトリ(Hugo の `content/` に相当するもの)に置く規約は無く、任意パスの Markdown ファイルを引数で渡す。ワークスペース(`noet init` で作る `.noet/`)はテンプレート置き場と `config.toml`(値は現行コードから未読込。下記参照)を持つだけで、記事ファイルの置き場所は関与しない。したがって NotePublisher は Zenn/Hugo/Jekyll のような固定パス規約を持たず、状態 JSON の `artifactPath` に「毎回好きな一時パス(または `note.workspace` 配下の `<uuid>.md`)」を記録すれば足りる
- **認証(§13-4。設計変更が必要)**: `noet` は note.com への認証を**自分では一切管理しない**。認証状態は「その時ブラウザ拡張機能が開いている実ブラウザが note.com にログイン済みか」で決まり(`noet auth` は `background.js` の `scrapeLoginStatus()` が note.com のトップページの DOM を見て判定するだけ)、環境変数・トークン・cookie を渡す手段は現行コードに存在しない。**現行の設定スキーマ `note: { workspace }` に `*_env` を追加しても認証は成立しない**。無人実行に必要な「サーバー上で完結する認証」の余地が無い、という結論(§13-4)
- **記事 ID の受け渡し(§13-4)**: `noet create` の成功レスポンスは公開時のみ最終 URL(`https://note.com/<user>/n/<key>` 形式)を返し、下書き保存時は URL も key も返さない(`apps/extension/src/background.js:280-333`)。したがって新規作成直後の `remoteId` 確定は「公開時は URL から key を正規表現抽出」「下書き時は `noet list` を実行して DOM スクレイピングされた記事一覧からタイトル一致で照合」のいずれかに頼らざるを得ない(§5.7 冒頭の「応答不明時の重複防止」規約と同じ照合ロジックを、note.com では**下書きのたびに**使うことになる)。`noet update <key> <file>` の `<key>` は事前にこの方法で取得した値を渡す
  - **照合の安全条件**: `noet list` は `/notes` ページの DOM スクレイプであり、ページング・遅延読み込みにより**一覧が完全である保証が無い**。不完全な一覧に対する「0件」を「未作成」と判定すると重複作成につながるため、「0件なら新規作成、1件なら ID 採用」の規則は**一覧の完全性(または検索結果の一意性)が確認できた場合のみ**適用する。取得範囲が不明・不完全な場合は当該ノートを failed とし状態を更新しない(update / delete の事前照合にも同じ判定を用いる)。完全性の確認方法(件数表示との突合・全件読み込みの完了検知等)は T-25 で noet の実装に即して確定する
- **レート制御(§13-4。設計前提の訂正)**: README.md や CHANGELOG.md が述べる「500ms 固定のレート制限」は API 直叩き時代の実装(`NoteClient`)の記述であり、現行ソースに rate limiter は存在しない(`rate_limit` / `RateLimit` で grep してヒットなし)。代わりに `background.js` の DOM 操作全体に、ボット検知回避を目的とした乱数遅延(`randomDelay(300, 5000)` 相当。`humanPageLoadWait`/`humanActionPause` 等)が散在しており、**1回の `create`/`update` はブラウザのページ遷移込みで実測十数〜数十秒かかる設計**。§6 のサブプロセス共通タイムアウト(その他ツール5分)は1回あたりには足りるが、拡張機能側の1コマンドあたりタイムアウトは60秒(`extension_client.rs:19` `COMMAND_TIMEOUT`)であり、画像を含む記事では複数回の待機(`waitForCondition` 系、最大15秒×画像数)が積み重なり60秒に接近し得る点に注意
- **画像(§13-6)**: R2 / S3 の外部公開 URL を Markdown の `![]()` のまま本文に残しても、note.com の編集画面(ProseMirror)は画像記法を解釈せず**リテラルなテキストとして表示される**(noet 自身の実機調査 `docs/IMAGE_UPLOAD_INVESTIGATION.md` / `docs/MARKDOWN_SUPPORT_TEST.md`。§13-6)。noet 側は「先に noet 自身が画像を note.com にアップロードしてから、本文中の参照をその URL に置換する」実装(`create --images` 経路)を用意しているが、これも「統合テスト未実施」(`docs/IMAGE_FEATURE_STATUS.md:109-141`)の未検証機能である
- **推奨対応(結論)**: 上記により、note.com は他サービス(Git 系・Qiita・dev.to・はてな)と異なり**ヘッドレス / 無人実行が構造的に不可能**(実ブラウザに人間がログインしている必要がある)。T-25 では次のいずれかを選択する必要があり、design.md の該当箇所(§4・§6・§7・本節)は選択結果に応じて改訂すること:
  1. **note.com を `sync` の自動配信対象から外し**、Renderer までの成果物(frontmatter 付き Markdown。§9)を `note.workspace` に書き出すだけの「半自動」モードとする(実際の `noet create`/`update` 実行と画像アップロードは利用者が手動で行う)
     - **画像の入力契約(§13-6 を踏まえた必須の取り決め)**: 半自動モードでも、AssetUploader が生成した R2/S3 外部 URL の `![]()` をそのまま本文に残すと note.com 上でリテラル文字列として表示されてしまう(§13-6)。したがって T-25 では次のどちらかを選択し、`![]()` がそのまま本文に残る実行結果を許さないこと: **(a)** note.com 向けでは AssetUploader の外部 URL 化を行わず、添付をローカルファイルとして書き出して本文参照をローカルパスに置換し、利用者が `noet create --images` 経路(noet 側で note.com へアップロード後に参照置換する実装。ただし「統合テスト未実施」の未検証機能)で入稿する / **(b)** 画像を含むノートは note.com 向けでは明示的に failed とし、エラーメッセージで非対応を伝える(noet の `--images` が成熟するまでの安全側の既定)
  2. あるいは note2web 側では関与せず、`sync` から `noet` の呼び出しを完全に除外し note.com 対応を見送る(将来 noet が API ベースの無人実行をサポートした時点で再スパイクする)
  - どちらの場合も、**現行設計が前提としていた「他 Publisher と同様の完全無人 `sync`」は note.com には適用できない**ことをドキュメント(README 等)に明記する必要がある
...
| note | `noet` バイナリに加え、**同一マシン上で note.com にログイン済みの実 Chrome ブラウザ + noet 拡張機能が起動していること**(§13-4)。トークン等の環境変数では代替できず、`doctor` の自動チェックで確認できる項目ではない(拡張機能との WebSocket 接続失敗は `sync` 実行時に初めて判明する)。無人実行の可否そのものが未解決のため、依存チェックの実装方針は §5.7 NotePublisher の「推奨対応」の選択後に確定する |
...
**4・6(note.com / noet)はさらに深刻で、「未決事項の解消」に留まらず §1 の実行モデル(cron / launchd からの無人実行)そのものが note.com には適用できないという結論に至った**。noet(commit `e3a8562`)は README.md / CHANGELOG.md が説明する「環境変数認証 + 非公式 API 直叩き」のアーキテクチャから、現行ソースでは「ログイン済みの実ブラウザ + Chrome 拡張機能を DOM 操作で遠隔操作する」アーキテクチャへ全面移行済みであり、この移行後のアーキテクチャにはサーバー上で完結する認証手段が存在しない(§13-4)。加えて note.com の編集画面は Markdown の画像記法を解釈しないため、外部 URL をそのまま埋め込む当初設計はリテラルテキストの露出という形で失敗する可能性が高い(§13-6)。**T-25 に着手する前に、§5.7 NotePublisher 末尾に記載した2つの対応案(a. note.com を無人配信の対象から外し半自動モードにする / b. note.com 対応を見送る)のいずれを採るかを決定する必要がある**(本タスクの範囲では推奨案の提示に留め、決定・実装(`src/config.ts` を含む)は T-25 で行う)。
