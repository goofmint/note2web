...
- **`noet` コマンド自体の解決先(`NOET_PATH`、必須。実機報告)**: `noet` は `cargo install` で導入されることが多く、その場合 `~/.cargo/bin/noet` に置かれる。`note2web init` が生成する launchd の plist の PATH(`buildLaunchdPath`、`src/init.ts`)は rbenv/asdf/rvm の shim と OS 標準ディレクトリのみを対象にしており `~/.cargo/bin` を含まないため、対話シェルの PATH には `~/.cargo/bin` が通っていても launchd(無人)実行だけ `required command "noet" was not found on PATH` で失敗する。この解決には PATH 探索の拡張ではなく、環境変数 `NOET_PATH` に `noet` バイナリの絶対パス(`~` 展開に対応)を持たせる方式を採る(`resolveNoetCommand`、`src/publishers/note.ts`)。`NOET_PATH` は `note2web init` が対話で尋ね(既定 `~/.cargo/bin/noet`)、env ファイルへ値入りで書き込む(FR-30: トークン等の秘匿情報とは異なり `NOET_PATH` は秘匿情報ではないため、値をエラーメッセージに含めてよく、init が値入りで書ける)。**`NOET_PATH` が未設定・空文字の場合、`noet` が PATH 上にあっても PATH へはフォールバックしない**——対話シェルでは偶然動作し launchd 環境でだけ壊れるという不可視の環境依存を再生産しないための意図的な仕様であり、`checkDependencies`(§6 依存表)が `doctor`/`sync` 冒頭で明確な問題として検出し、NotePublisher 自身も `publish()` 時点で同じ理由により例外を投げる(依存チェックをバイパスした呼び出し経路への防御)。加えて、`~` 展開後の値が**絶対パスでない場合も同様に拒否**する(相対パスは cwd 依存で対話シェルと launchd の間で解決先が変わるため。`init` も絶対パスが入力されるまで問い直す)。依存チェック側は値が指す先が「実行可能な通常ファイル」(regular file + `R_OK | X_OK`)であることまで確認する——ディレクトリや実行権限の無いファイルの指定を実際の `noet` 起動失敗より手前で検出するため(PR #84 レビュー)
...
- **画像(§13-6。決定変更: option (b) → option (a)、利用者決定 2026-08-21)**: T-25 時点では、R2 / S3 の外部公開 URL を Markdown の `![]()` のまま本文に残しても note.com の編集画面(ProseMirror)は画像記法を解釈せず**リテラルなテキストとして表示される**(noet 自身の実機調査 `docs/IMAGE_UPLOAD_INVESTIGATION.md` / `docs/MARKDOWN_SUPPORT_TEST.md`。§13-6)ことを理由に、noet 自身の画像アップロード経路が「統合テスト未実施」(`docs/IMAGE_FEATURE_STATUS.md:109-141`)の未検証機能であることから、より安全な (b)(画像を含むノートを note.com 向けでは明示的に failed とする)を採用していた。**2026-08-21、利用者が実機で画像入りノートを note.com へ配信する必要が生じたため、(a)(noet 自身の画像アップロード機能を、ローカルファイル参照で使う)へ方針転換した**。仕組みは次のとおり(kako-jun/noet commit `e3a8562` のソース確認に基づく):
  - `noet create <file>`/`noet update <key> <file>` は実行時、本文 Markdown 中の画像参照を `extract_image_references`(正規表現 `!\[([^\]]*)\]\(([^)]+)\)`)で自動検出する。alt テキストがそのまま note.com 側のキャプションになる
  - 参照パスは `markdown_path.parent()`(= Markdown ファイル自身が置かれたディレクトリ)基準で解決される(`base_dir`)。したがって `http://`/`https://` で始まる参照(R2/S3 の公開 URL 等)は**アップロード対象から除外(スキップ)**される——ProseMirror が `![]()` を解釈しないという上記の制約は変わらないため、外部 URL 参照はそのまま本文に残り無意味になる。**画像はローカルファイル参照(相対パス)でなければならない**
  - サポートされる画像形式は拡張子ベースで jpg/jpeg/png/gif/webp のみ(`read_image_as_base64`)。それ以外の拡張子は `noet create`/`update` のコマンド全体を失敗させる
  - noet がアップロードごとに note.com が発行する `st-note.com` URL を取得し、本文中の参照をそのアップロード後 URL に書き換えてから記事を公開する。1コマンドあたりの拡張機能側タイムアウトは60秒(`extension_client.rs` `COMMAND_TIMEOUT`)であり、画像を多数含む記事ではこれに接近しうる(上記「レート制御」項と同じ注意点)
  - この機能自体は upstream で「統合テスト未実施」のままであり、note2web 側でも実機検証はできていない(§12 参照)。失敗した場合は他の失敗と同様にそのノートのみが `'failed'` として隔離され、次回実行で再試行される(NFR-06)——「画像を含むノートは常に failed」という以前の (b) の挙動には戻らない

  **note2web 側の実装**: `assets/uploader.ts` の `processNoteBody` が、`service === 'note'` かつプレースホルダが画像(`isImageExtension`)を指す場合に、R2/S3 アップロード経路の代わりにファイルを `<config.note.workspace>/images/<identifier>-<内容ハッシュ先頭16桁><ext>` へコピーし、本文の参照を `./images/<identifier>-<内容ハッシュ先頭16桁><ext>` という相対パスに差し替える(ファイル名に内容ハッシュを含めるのは、同じ identifier のまま画像だけを差し替えた場合にも本文 Markdown → contentHash が変わり、既存のスキップ判定のまま `noet update` による再配信が発動するようにするため——R2 経路がキーに内容ハッシュを使うのと同じ性質。identifier は UUID の文字集合(英数字とハイフン)であることを検証したうえでファイル名に使い、解決後のコピー先が `images/` 配下に収まることも防御的に確認する)(NotePublisher が記事 Markdown を `<workspace>/<uuid>.md`(ワークスペース**直下**)に書き出すため、`./images/...` は Markdown ファイルの親ディレクトリからの相対パスとして正しく解決される)。noet がサポートしない拡張子は、`noet` の60秒タイムアウトの末に不可解に失敗させるのではなく、このアセット解決段階で明確な `AssetUploadError` として早期に検出する(`assets/uploader.ts` の `NOET_SUPPORTED_IMAGE_EXTENSIONS`)。ローカルコピーは使い捨てであり `StateStore` のアセット状態には記録しない(実行のたびに再コピーする。決定的な上書きのため再実行しても問題ない)。`<workspace>/images/` 配下に残る、もはやどのノートからも参照されなくなった古いファイルは掃除しない——noet は毎回そのノートが実際に参照する画像だけを再アップロードするため無害であり、note.com 側の未参照アップロードの扱いは note.com 自身に委ねる。旧 `NoteImagesUnsupportedError`(画像を検出して一律拒否するエラー型)は廃止した——**対応形式(jpg/jpeg/png/gif/webp)の添付画像**はもはや note.com 向けの失敗理由ではない。引き続き失敗になるのは、(1) 非対応拡張子の画像添付(前述の `AssetUploadError` による早期検出)と、(2) 外部 URL(`http(s)://`)を指す画像参照——添付を伴わない `<img src="外部URL">` 由来で、noet がアップロード対象からスキップするため公開するとリテラル表示になる——の2つのみで、後者は `renderNoteArticle` が `NoteExternalImageError` として拒否する
...
| note | `NOET_PATH`(`noet` バイナリの絶対パスを指す環境変数、実機報告。`checkDependencies` が未設定/空・パス不在の両方を problem として報告し PATH フォールバックは行わない。§5.7 NotePublisher「`noet` コマンド自体の解決先」参照)に加え、**同一マシン上で note.com にログイン済みの実 Chrome ブラウザ + noet 拡張機能が起動していること**(§13-4)。後者はトークン等の環境変数では代替できず、`doctor`/`sync` 冒頭の自動チェックで確認できる項目ではない(拡張機能との WebSocket 接続失敗は `noet` サブプロセス実行時に初めて判明し、当該ノートのみ failed になる。§5.7 NotePublisher「認証・実行モード」参照)|
